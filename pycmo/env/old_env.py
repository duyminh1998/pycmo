...
class TimeStep(collections.namedtuple('TimeStep', ['step_id', 'step_type', 'reward', 'discount', 'observation'])):
  """
  Description:
  ...
  Attributes:
    ...
    discount: A discount value in the range `[0, 1]`, or 0 if `step_type` is `StepType.FIRST`, i.e. at the start of a sequence.
...
class CMOEnv():
  ...
  def reset(self) -> TimeStep:
    ...
    f.write('False')
    return TimeStep(0, StepType(0), 0, 0, self.get_obs(0)) # return initial time step
